 ...
 * status even if Layers 1+2 were bypassed.
 *
...
export async function POST(req: Request) {
  ...
  try {
    Sentry.addBreadcrumb({
      category: "webhook",
      level: "info",
      message: "plaid_webhook_entry",
      data: { route: "plaid_webhook" },
    });

    const result = await adapter.handleWebhook(raw, headers);
    if (!result.verified) {
      captureMessage("plaid_webhook_invalid_signature", "warning", {
        route: "plaid_webhook",
      });
      return NextResponse.json(
    ...
    // Layer 3 jurisdiction guard — Iron Law 2.31.
    if (result.intent) {
      try {
        await assertOrderJurisdictionAllowed(result.intent);
      } catch (err) {
        if (err instanceof JurisdictionalGuardError) {
          captureMessage("plaid_webhook_jurisdiction_blocked", "warning", {
            route: "plaid_webhook",
            reason: err.message,
          });
          return NextResponse.json(
    ...
    Sentry.addBreadcrumb({
      category: "webhook",
      level: "info",
      message: "plaid_webhook_reconciled",
      data: {
    ...
    captureException(err, {
      tags: { route: "plaid_webhook", provider: "plaid" },
    });
    return NextResponse.json(
