# Stage 6 Super-Prompt v3.0: Peptide D2C E-Commerce Build

Version: 3.0
Date: 2026-05-08
Target: Claude Code CLI session, Claude Opus model
Predecessors: SUPER_PROMPT_2026-05-08.md (v1.0), SUPER_PROMPT_v2_2026-05-08.md (v2.0)

Why v3.0: v2.0 established the architectural base (NEW repo not Mogtrix-fork, full corpus read mandate, Subagent Constitution, hard performance/UX/a11y targets, 14 Iron Laws). v3.0 hardens v2.0 with patterns drawn from two repos audited for this purpose: `nextlevelbuilder/ui-ux-pro-max-skill` (UI design intelligence; ported 4 patterns: reasoning-rules architecture for vendor archetypes, verbatim font imports, motion-vocabulary precision, pre-delivery checklist) and `affaan-m/everything-claude-code` (battle-tested orchestration; ported 8 patterns: TDD checkpoint commits with git-evidence protocol, pre-commit supply-chain scanner, formal skill-adaptation policy, optional memory-persistence hooks, worktree cascade method, structured hook observability, security threat model with CVE references, agent-introspection-debugging when 3+ fixes fail). Three new Iron Laws (2.15-2.17) enforce these. Four new appendices (T-W) carry verbatim content. Operator-decided: single-track Opus model for all work (no Haiku/Sonnet downgrade for cost) — build is too high-stakes to risk a smaller model on any task. The result is additive anti-drift hardening + design-system grounding + security threat-model framing without scope creep into either repo's broader catalogs.
...
**2.16 PRE-COMMIT SUPPLY-CHAIN SCANNER MANDATORY.** The pre-commit hook (Phase 8 setup) extends `assertMarketingCopySafe` with a full supply-chain scanner. The hook MUST run on every commit and fail-fast on any hit. Scanner contents (per Appendix U):
- Hidden unicode: zero-width spaces, word joiners, bidi override characters, BOM
- Forbidden infrastructure keywords: `curl | bash`, `wget | bash`, `enableAllProjectMcpServers`, `ANTHROPIC_BASE_URL`, `--dangerously-skip-permissions`, `--no-verify` (in scripts), suspicious base64 blobs > 200 chars
- File-permission violations: any `.env*` file (except `.env.example`), any `id_rsa*` or `*.pem`, any `*credentials*` filename
- Console.log / debugger / TODO-without-issue-link in production source paths (`app/`, `lib/`, `components/`)
- HTML comment patterns suggesting prompt injection: `<!--` near `[A-Z]{4,}`, `data:text/html`, embedded scripts in markdown
- Forbidden marketing words from `lib/compliance.ts` `assertMarketingCopySafe`
This is non-negotiable infrastructure security. The hook is a compile-time gate.
...
4. Emit a structured introspection report to `docs/checkpoints/introspection_<phase>_<timestamp>.md`
...
When phases produce 3+ truly independent modules that risk git conflicts, use the worktree cascade pattern (per `affaan-m/everything-claude-code` shortform guide):

```bash
cd <new-project-dir>
git worktree add ../module-a feature/module-a
git worktree add ../module-b feature/module-b
git worktree add ../module-c feature/module-c
```
...
4. Remove worktrees after merge: `git worktree remove ../module-a`

This pattern works best when modules are truly orthogonal (e.g., payment adapter + email service + COA hosting). It does not work for tightly-coupled modules (e.g., catalog UI + catalog API). Use sequential subagent dispatch for coupled modules.

### 4.5 Model selection (single-track Opus only)

Every task in this build runs on Claude Opus (currently `claude-opus-4-7` per environment). This is a deliberate operator decision: the build is high-stakes (compliance + payment + brand), zero-edit-deployable bar, and survives FDA enforcement only with maximum reasoning quality. We do NOT route tasks to Haiku or Sonnet for cost optimization. The cost of a regression caught only by the operator post-deploy exceeds the token savings.
...
For sessions that span >4 hours or multiple working days, install three Claude Code hooks (per `affaan-m/everything-claude-code` longform guide). These automate session-continuity without requiring manual `/context-save`:

**PreCompact hook** (fires before context compaction):
- Saves the current phase, locked decisions, recent test outputs, and outstanding issues to `~/.claude/sessions/<date>/precompact_<timestamp>.md`
- Lets the agent recover state if compaction loses earlier context
- Implementation: settings.json hook config calling a small bash script that captures git status + active TodoWrite items + last 10 commits + recent test command output

**SessionStart hook** (fires when a new session opens in the project directory):
- Auto-loads the most recent session summary from `~/.claude/sessions/<project-slug>/`
- Writes a brief "resuming from <last-phase>, last completed: <last-commit>, outstanding: <todo-list>" preamble to context
- Saves the operator from manually running `/context-restore`

**Stop hook** (fires once at session end, NOT on every message):
- Persists session learnings to `docs/learnings/<date>.md`: what worked, what surprised the agent, patterns observed in the research that informed implementation choices
- Captures any "I had to revisit X assumption" moments
- Does NOT block session-end; runs async, idempotent

These hooks are OPTIONAL. The base v3.0 workflow runs without them. Add them when the operator's session-management overhead becomes a bottleneck.

### 5.9 Structured hook observability

Every hook (whether PreCommit, PreToolUse, PostToolUse, PreCompact, SessionStart, Stop) writes a structured JSON line to `~/.claude/sessions/hooks.jsonl`:

```json
{
  "timestamp": "2026-05-08T14:23:11Z",
  "session_id": "<sha256>",
  "hook_id": "pre-commit-supply-chain",
  "trigger": "git commit",
  "decision": "allow | block | warn",
  "checks_run": ["hidden_unicode", "forbidden_keywords", "marketing_copy_safe"],
  "violations": [],
  "duration_ms": 47
}
```

This makes hook behavior auditable. When a hook misfires or starts blocking unexpectedly, the operator can grep the log instead of forensic-reading hook scripts. Per `affaan-m/everything-claude-code` security guide.
...
### Appendix T: Skill Adaptation Policy (porting from reference repos)

This project draws patterns from three reference repos: Mogtrix (peptide-RUO storefront patterns), `affaan-m/everything-claude-code` (orchestration + verification), `nextlevelbuilder/ui-ux-pro-max-skill` (design intelligence). When porting a pattern from any of these (or future reference repos), follow this policy verbatim.

**T.1 Copy the underlying idea, not the external product identity.**
- Yes: port the TDD checkpoint commit pattern from ECC; rename it for v3.0 context.
- No: reference ECC by name in user-facing copy or commit messages; do not advertise a third-party brand.

**T.2 Rename when the v3.0 surface changes substantially.**
- If we keep ECC's TDD checkpoint exactly as-is (including commit-message format), call it "TDD checkpoint" in the spec — operator knows the heritage from this appendix.
- If we change the format materially (e.g., add brand-voice-safety check to GREEN commit), rename it ("v3.0 TDD-with-compliance-checkpoint") so future maintainers know what diverged.

**T.3 Prefer v3.0-native rules, skills, scripts, MCPs over new third-party defaults.**
- We already have Superpowers + gstack as the tooling base. New patterns should call those skills, not introduce parallel tooling.
- Example: Iron Law 2.15 (TDD checkpoint commits) calls `superpowers:test-driven-development` for the test loop; we do not introduce a separate "ecc-tdd" skill.

**T.4 Do not ship a pattern whose main value is "install this external package."**
- ECC's `agent-sort` skill recommends installing 50+ external skills based on context. v3.0 does not adopt this; we curate a known-small set.
- If a pattern adds value only by referencing a third-party tool that the operator must install, skip it.

**T.5 One-line attribution comment in code.**
- Format: `// Pattern adapted from <repo>/<path>` (single line, in the file where the pattern is implemented).
- Do not embed the source repo's identity in product UI copy, ToS, marketing, or operator-facing artifacts.

**T.6 Document every adaptation in this Appendix T or a referenced ADAPTATION_LOG.md.**
- For v3.0, the adaptations are documented in this super-prompt (Appendices T-W) and in the §0 "why v3.0" header.
- For post-launch additions, append to `docs/ADAPTATION_LOG.md` with date + source repo + pattern + rationale + customizations.

**T.7 Audit upstream before porting.**
- Before porting a pattern, the operator (or agent on operator's behalf) reads the source file in full and verifies it does what the documentation claims.
- The Snyk ToxicSkills 2025 study found 36% of public skills contain prompt-injection patterns. Treat every external skill as untrusted source until audited.
- This is enforced by the pre-commit supply-chain scanner (Iron Law 2.16), which scans imported patterns for hidden unicode and forbidden keywords.

### Appendix U: Security Threat Model (with CVE references)

This appendix is grounded in `affaan-m/everything-claude-code/the-security-guide.md` (~29KB) and serves as the framing for Iron Law 2.5 (`/cso` gate), Iron Law 2.16 (supply-chain scanner), and the LLC-isolation posture in `DECISIONS/compliance_posture.md`.

**U.1 Known-CVE threat surface (as of 2026-05-08)**

- **CVE-2025-59536**: Pre-trust execution vulnerability where Claude Code executed remote-fetched scripts before user approval. Mitigation: never approve `curl | bash` patterns; pre-commit scanner blocks the literal string per Iron Law 2.16.
- **CVE-2026-21852**: ANTHROPIC_BASE_URL hijacking allowing redirect to attacker-controlled inference endpoints. Mitigation: pre-commit scanner blocks `ANTHROPIC_BASE_URL` in any committed config or script; operator never sources unfamiliar `.env` files.
- **Snyk ToxicSkills 2025 study**: 36% of public Claude Code skills contained at least one prompt-injection pattern (hidden unicode, comment payloads, base64-buried instructions). Mitigation: every external pattern is audited before porting per Appendix T.7.

**U.2 Prompt injection attack surface**

Everything an LLM reads is executable context. There is no meaningful distinction between "data" and "instructions" once text enters the context window. The pre-commit scanner (Iron Law 2.16) checks for:

- Zero-width unicode: `\x{200B}` (ZWSP), `\x{200C}` (ZWNJ), `\x{200D}` (ZWJ), `\x{2060}` (WJ), `\x{FEFF}` (BOM)
- Bidi override characters: `\x{202A}` through `\x{202E}` (LRE, RLE, PDF, LRO, RLO)
- HTML comment payload patterns: `<!--` near `[A-Z]{4,}` (often masking instructions)
- Embedded script blocks in markdown: `<script`, `data:text/html`
- Suspicious base64 blocks: `>200 char base64-encoded text in source files outside test fixtures`

Reference grep patterns:
```bash
rg -nP '[\x{200B}\x{200C}\x{200D}\x{2060}\x{FEFF}\x{202A}-\x{202E}]'
rg -n '<!--|<script|data:text/html|base64,'
```

**U.3 Infrastructure attack surface**

- Curl-piped-bash patterns: `curl | bash`, `wget | bash`, `curl ... | sh`. Always blocked.
- Force-skip flags in scripts: `--no-verify`, `--dangerously-skip-permissions`, `enableAllProjectMcpServers`. Always blocked.
- Permission elevation: any reference to `sudo`, `chmod 777`, broad permission changes. Audit before approval.
- Outbound network commands without explicit operator approval: `nc`, `scp`, `ssh`, `rsync`. Blocked in pre-commit hook for new files; allowed via `// SCANNER_OK: <reason>` annotation if operator-approved.

**U.4 Supply-chain attack surface (this project's specific exposure)**

- Mogtrix codebase: read-only reference per Iron Law 2.12. Pre-commit grep test fails build if "Mogtrix" appears in source without attribution comment.
- Research corpus: trusted (locally-controlled, read-only). No injection risk.
- Third-party libraries: standard `npm audit` + `npm install --ignore-scripts` for new packages. Operator reviews lock-file changes before commit.
- Skill / plugin imports from public Claude Code marketplaces: NOT permitted in v3.0 build. Curated set of skills is fixed (Superpowers + gstack). Adding new skills requires Appendix T audit.

**U.5 Compliance + LLC isolation posture (operator-side, NOT v3.0 build-side)**

These are operator decisions reflected in `DECISIONS/compliance_posture.md` and `DECISIONS/source_terms.md`. v3.0 surfaces them as part of the build:

- LLC formation in defensive jurisdiction (Wyoming preferred; Delaware or Nevada acceptable).
- Manager-member structure: holding-company manager isolates operator's personal name from public LLC records.
- Domain registration: registered to LLC, not personal name. Privacy WHOIS enabled.
- Choice-of-law clause in ToS: same jurisdiction as LLC.
- No personal-name disclosure on the site (no "About the Founder" with personal photo on Day 1).

**U.6 Active monitoring posture (post-deploy, Phase 15)**

- Sentry alert thresholds: error rate > 1% triggers email; payment-flow error rate > 0.1% triggers immediate page.
- gstack `/canary` monitors for 2 hours post-deploy; structured `hooks.jsonl` retained for 90 days.
- Weekly `/retro` runs; quarterly compliance review against `enforcement_events.md` for new FDA enforcement patterns.
- Operator monitors FDA Inspections database + Drug Safety Communications RSS feed for category-relevant enforcement.

### Appendix V: Vendor-Archetype Design Reasoning Rules

Pattern adapted from `nextlevelbuilder/ui-ux-pro-max-skill/src/ui-ux-pro-max/data/ui-reasoning.csv`. The original maps 161 generic product types to design recommendations. v3.0 adapts the schema for peptide-vendor archetypes derived from the 208-vendor anatomy research.

**V.1 Schema** (per row):

| Column | Description |
|---|---|
| Vendor archetype | Posture A (Clean Clinical) / Posture B (Meme-Coded Community) / Cross-Posture / Specific outliers (e.g., "Domestic Supply gray-market", "Healthgevity nutraceutical-positioned") |
| Recommended pattern | Site IA pattern observed in archetype (e.g., "grid-disciplined catalog with research-frame copy") |
| Style priority | Visual-language priority (e.g., "minimal whitespace, scientific gravitas, NO meme-coded imagery") |
| Color mood | Color palette direction (e.g., "charcoal grey + teal accent + white space") |
| Typography mood | Typography direction (e.g., "IBM Plex Sans + Plex Mono + Newsreader Italic editorial moments") |
| Key effects | Motion / interaction vocabulary (e.g., "vial sway 6.4s ease-in-out, hover unfurl 200ms, stagger reveal 60-80ms") |
| Decision rules | Conditional adjustments per context (e.g., "if catalog > 15 SKUs: add filter sidebar; if mobile: collapse to bottom sheet") |
| Anti-patterns | Explicit visual exclusions per archetype (e.g., "no purple/violet gradients, no 3-column SaaS feature grid, no stock-photo lab interiors, no vibrant block-based e-commerce template") |
| Severity | HIGH / MEDIUM / LOW for the rule (severity gates `/design-review` blocking behavior) |

**V.2 Posture A (Clean Clinical Labs) — full row**

| Field | Value |
|---|---|
| Vendor archetype | Posture A: Clean Clinical Labs (Numerus Labs default; Bezel Bio, Cubit Labs, Numerus Labs as named candidates) |
| Recommended pattern | Grid-disciplined catalog + asymmetric vial-dominant home + dense COA tables + research-paper-tone blog index |
| Style priority | Premium dark + density + intentional decoration (radial gradients, grain overlay) + research-instrument aesthetic + scientific gravitas |
| Color mood | `--bg #0a0e0f` charcoal, `--surface #141a1c`, `--accent #3dd4c8` teal (rare CTA only), `--electric #67e8f9` cyan (atmospheric secondary), `--text rgba(255,255,255,0.92)`, `--text-muted rgba(255,255,255,0.62)`. WCAG AA verified pairs only. |
| Typography mood | IBM Plex Sans (300/400/500/600/700) primary, IBM Plex Mono (300/400/500/600) for SKUs and data, Newsreader Italic (400i) for hero pull-quotes only. Tabular numerals always. |
| Key effects | Vial sway -12° to +12° / 6.4s ease-in-out infinite (NOT full rotation), vial float 7.6s, vial sheen 5.4s offset per-vial, hover unfurl 200ms ease-out, stagger reveal 60-80ms / 320ms duration, page transition 540ms View Transitions API. Reduced-motion fallback hard-disables. |
| Decision rules | If catalog > 15 SKUs: add filter sidebar with category/dose/format/in-stock toggles. If mobile: collapse vial scenes to 60-70% viewport. If `prefers-reduced-motion: reduce`: disable all continuous animation. If first visit: show age-gate text-checkbox at first cart action (NOT modal popup). |
| Anti-patterns | NO Geist / Inter / Roboto / Space Grotesk primary fonts (convergence trap), NO purple/violet gradients, NO 3-column SaaS feature grid, NO bubble-radius on every element, NO stock-photo lab interiors, NO "Built for X" SaaS marketing copy, NO before/after photography, NO vibrant block-based e-commerce template, NO emoji icons (SVG via Lucide only). |
| Severity | HIGH for typography + color + anti-patterns; MEDIUM for motion (reduce-motion fallback covers degradation); LOW for decision-rule conditional sidebar (UX optimization). |

**V.3 Posture B (Meme-Coded Community) — full row**

| Field | Value |
|---|---|
| Vendor archetype | Posture B: Meme-Coded Community (Skullcap Labs, Pylon Peptides, Voltic Peptides candidates) |
| Recommended pattern | High-contrast catalog + looksmax-coded hero photography + creator-influencer affiliate-grid module + community-research blog tone |
| Style priority | High-contrast dark + neon accent + athletic / anatomical illustration + brutalist concrete (Pylon variant) + post-ironic register (handled via copy, not visual gimmickry) |
| Color mood | Deeper black (`--bg #050505`) + neon cyan or magenta accent + cool-white text. No earth tones. |
| Typography mood | Sans-serif primary with stylized weight (e.g., Space Grotesk 600 or custom display font), monospace for product codes, optional serif italic for editorial moments. |
| Key effects | Vial scenes optional but allowed; if used, more aggressive parallax. Hover scale 1.02-1.05 (slightly more pronounced than Posture A's 1.00 unfurl). Stagger reveal 30-50ms per item (faster than Posture A). |
| Decision rules | If catalog > 15 SKUs: same filter pattern as Posture A. If creator-affiliate page: lead with creator-tier ranking from `slice_B2_influencer_tier_map.md`. If community engagement low: surface "joining the research community" CTA more prominently. |
| Anti-patterns | Same anti-patterns as Posture A (no purple gradients, no 3-column SaaS, no stock photos, no emoji icons). PLUS: NO meme-imagery in product photography (vials only); NO ironic compliance copy ("LOL we're totally not for human use" patterns); NO race-to-the-bottom pricing language ("CHEAPEST", "DEAL"). |
| Severity | HIGH for typography + anti-patterns; MEDIUM for motion + color (less critical for compliance, more critical for brand consistency); LOW for decision-rule conditionals. |

**V.4 Cross-Posture (Bezel Bio default for cross-posture)**

| Field | Value |
|---|---|
| Vendor archetype | Cross-Posture (operator may toggle visual register over time; Bezel Bio, Schema Peptides, Pylon Peptides, Codex Research, Hex Labs as named candidates) |
| Recommended pattern | Adaptive: Posture A primary surfaces (catalog, COA library, ToS) + Posture B-friendly surfaces (community blog, affiliate page) |
| Style priority | Instrument-grade neutral by default, can lean clinical or community based on page context |
| Color mood | Charcoal + brushed-metal accent + restrained color palette that reads either as "watch / lab instrument" or "skate / athletic" depending on visual treatment |
| Typography mood | Inter or IBM Plex Sans primary, JetBrains Mono for code, optional editorial italic |
| Key effects | Posture A motion vocabulary by default; Posture B available as opt-in per-page |
| Decision rules | Per-page posture: catalog/PDP/COA = Posture A; blog/affiliate/community = Posture B-friendly. Brand voice consistent across both. |
| Anti-patterns | All Posture A anti-patterns (universal). Plus: do NOT mix the two postures within a single page (no meme caption on a clinical product page; no clinical disclaimer block in a meme-coded blog post — both must hold the brand voice consistently). |
| Severity | HIGH for typography + universal anti-patterns; MEDIUM for visual-register-per-page rules; LOW for adaptive treatments. |

**V.5 How the agent uses this appendix**

- During Phase 4 (Brand and Design System), agent reads the row matching the locked or auto-defaulted brand pick.
- The recommended pattern + style priority + color mood + typography mood become token values in `lib/design/tokens.ts` and `app/globals.css`.
- Anti-patterns become assertion checks in `/design-review` (any violation surfaces in review report).
- Decision rules become conditional logic in component composition.
- Severity informs blocking behavior: HIGH violations block phase progression; MEDIUM violations surface in review notes; LOW violations are noted but not blocking.

### Appendix W: Pre-Delivery Checklist (per UI/UX Pro Max pattern)

Pattern adapted from `nextlevelbuilder/ui-ux-pro-max-skill/.claude/skills/ui-ux-pro-max/SKILL.md`. Run this checklist before claiming any phase that produces user-facing surfaces is complete (Phases 4, 5, 6, 7, 10, 12). Failed items block the phase verification gate.

**W.1 Visual Quality**
- [ ] No emojis as icons (SVG only via Lucide React)
- [ ] All icons from consistent family (stroke width, corner radius)
- [ ] Brand wordmark used with correct proportions and clear-space (per Phase 4 brand assets)
- [ ] Pressed-state visuals do not shift layout bounds (transform-only, no width/height/top/left animations)
- [ ] Semantic theme tokens used consistently (no ad-hoc hex colors in component source)
- [ ] No anti-pattern fonts (Geist, Inter, Roboto, Space Grotesk for Posture A primary)
- [ ] No anti-pattern color treatments (purple/violet gradients, 3-column SaaS feature grid)
- [ ] No stock-photo lab interiors

**W.2 Interaction**
- [ ] All clickable elements show pressed feedback within 100ms
- [ ] Touch targets meet minimum size (44x44px on mobile, 48x48px Android)
- [ ] Hover states 200-300ms with `cubic-bezier(0.16, 1, 0.3, 1)` premium-out easing
- [ ] Disabled states visually clear and non-interactive (opacity 0.5 + cursor not-allowed)
- [ ] Focus rings 2px solid accent + 2px offset, never `outline: none` without replacement
- [ ] Gesture regions non-conflicting (no tap+drag overlap)
- [ ] Loading states use skeleton screens above the fold; spinners only below

**W.3 Light/Dark Mode (if applicable)**
- [ ] Primary text contrast ≥4.5:1 in both light and dark mode
- [ ] Secondary text contrast ≥3:1 in both light and dark mode
- [ ] Dividers/borders distinguishable in both modes
- [ ] Modal scrim 40-60% black opacity
- [ ] Both themes tested before delivery
- [ ] Theme toggle persists in localStorage with `prefers-color-scheme` first-load default

**W.4 Layout + Responsive**
- [ ] Safe areas respected (notch, home indicator)
- [ ] Scroll content not hidden behind fixed/sticky bars
- [ ] Verified at 375px, 768px, 1024px, 1440px viewport widths
- [ ] Adaptive horizontal gutters by device size
- [ ] 4/8px spacing rhythm maintained across component / section / page levels
- [ ] Long-form text readable on large devices (no edge-to-edge paragraphs at >1200px viewport)

**W.5 Accessibility + Motion**
- [ ] All meaningful images/icons have descriptive `alt` or `aria-label`
- [ ] Form fields have associated `<label>` (no placeholder-only labeling)
- [ ] Error messages reference field by name, use `role="alert"` for screen readers
- [ ] Color is NOT the sole indicator (status pills always include text)
- [ ] `prefers-reduced-motion: reduce` hard-disables all animation
- [ ] Keyboard navigation: every interactive element reachable via Tab; tab order matches visual order
- [ ] Skip-to-content link present in header
- [ ] Heading hierarchy sequential h1 → h2 → h3 (no level skip)
- [ ] Dynamic text size supported without truncation

**W.6 Compliance + Brand**
- [ ] `assertMarketingCopySafe` grep returns 0 hits in this phase's output files
- [ ] Pre-commit supply-chain scanner returns 0 violations
- [ ] Verbatim disclaimer block present in footer of every page (per Appendix A.1)
- [ ] Verbatim product-page disclaimer present on every product page (per Appendix A.2)
- [ ] Age gate text-checkbox present at first cart action (per Appendix A.3)
- [ ] Brand wordmark and tokens match Phase 4 locked design system
...
- [ ] Initial CSS bundle ≤ 80KB gzipped

The agent runs this checklist via gstack `/design-review` + `/qa` + `/benchmark` + `assertMarketingCopySafe` grep + Lighthouse CI. All seven sections must pass before phase verification gate signs off.
