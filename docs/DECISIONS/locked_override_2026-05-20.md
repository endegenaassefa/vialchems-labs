...
| **Jurisdictional block (AMENDED per supplemental S20)** | **No US state blocks; all 50 US states allowed.** International remains US-only. | **Iron Law 2.8 amendment.** Operator commit `db59d5e6 Apply launch transcript updates` deliberately weakened `BLOCKED_US_STATES` from `['CA','TX','NY','FL']` to `[]` + removed "Operator may NOT weaken" comment. Decision driven by 200K-impression ad campaign reach (4 most-populous-states accounted for ~30% of US population). Day-1 buyer-attests assumes regulatory responsibility per existing 7-attestation Appendix A.5. Phase 3 closure restores the "Operator may NOT weaken without LOCKED_OVERRIDE" comment + references this doc. Operator may re-add per-state blocks via separate `docs/DECISIONS/iron_law_2_8_block_<date>.md`. |
