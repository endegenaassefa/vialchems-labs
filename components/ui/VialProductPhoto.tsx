 ...
 * original vialchemlabs.net artwork; no competitor raster asset is used.
 */
...
function assertCompoundAllowed(compound: string): void {
  ...
  if (!allowedCompounds.has(normalized)) {
    throw new Error(
      `Compound "${compound}" is not in the vialchemlabs.net catalog.`,
    );
  }
}
...
export function VialProductPhoto({
  ...
  return (
    <div
      ...
      aria-label={`vialchemlabs.net ${compound} ${dose} batch ${batch} vial product photo`}
      {...rest}
      style={
        {
