...
const featured = ['bpc-157-10mg', 'tb-500-5mg', 'ghk-cu-50mg', 'semax-10mg', 'selank-10mg', 'mots-c-10mg']
      ...
      >
        <div>
          ...
          <h1 style={{ marginBottom: 24 }}>
            ...
            <em style={{ fontStyle: 'normal', color: 'var(--fg-muted)' }}>shipped with the COA.</em>
          </h1>
          <p style={{ fontSize: 18, color: 'var(--fg-muted)', maxWidth: 520, marginBottom: 32, lineHeight: 1.5 }}>
            Batch-traceable materials for verified research organizations. Every vial links back to its certificate of
            analysis, lot history, and release documentation.
          </p>
...
function FloatingCards() {
  ...
  const semax = catalogItems.find((item) => item.slug === 'semax-10mg') ?? catalogItems[0];
  ...
  return (
    <div style={{ position: 'relative', height: 560, perspective: 1200 }}>
      ...
      <div style={{ position: 'absolute', top: 20, left: 20, width: 240, animation: 'float-y 6s ease-in-out infinite' }}>
        <div className="card" style={{ padding: 14, boxShadow: 'var(--shadow-lg)' }}>
          ...
          <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 2 }}>{semax.shortName.toUpperCase()}</div>
          ...
          >
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 13, fontWeight: 500 }}>{displayPrice(semax.priceCents)}</span>
      ...
      >
        <div className="card" style={{ padding: 16, boxShadow: 'var(--shadow-lg)' }}>
          ...
          >
            ...
            >
              {verified ? <Icon.check size={11} strokeWidth={2} /> : <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#fff' }} />}
            </div>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: verified ? 'var(--ok)' : 'var(--fg-muted)' }}>
...
function Timeline() {
  ...
  useEffect(() => {
    const id = window.setInterval(() => setActive((value) => (value + 1) % (steps.length + 2)), 900);
    return () => window.clearInterval(id);
  }, [steps.length]);

  return (
    <div style={{ position: 'relative' }}>
      <div style={{ position: 'absolute', top: 8, left: 8, right: 8, height: 1, background: 'var(--line)' }} />
      <div
        style={{
          position: 'absolute',
          top: 8,
          ...
          <div key={step.label}>
            <div
              style={{
                ...
                height: 16,
                borderRadius: '50%',
                background: i <= active ? 'var(--accent-hi)' : 'var(--bg-elevated)',
            ...
            >
              {i <= active && <span style={{ width: 5, height: 5, borderRadius: '50%', background: 'var(--bg-elevated)' }} />}
            </div>
            <div style={{ fontSize: 11, fontWeight: 500, marginBottom: 2 }}>{step.label}</div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--fg-muted)' }}>{step.date}</div>
          </div>
        ))}
      </div>
    </div>
...
function HowItWorks() {
  const steps = [
    ['01', 'Get Verified', 'Submit organization details, role, and a research-use attestation. Reviewed before order access.'],
    ['02', 'Browse the Catalog', 'Full peptide catalog unlocks once your organization is verified.'],
    ['03', 'Order Your Peptides', 'Add vials to cart, accept the research-use terms, and check out through approved rails.'],
    ['04', 'Receive With COA', 'Every vial arrives with batch-specific certificate references and lot traceability.'],
    ['05', 'Verify Anytime', 'Scan the label or look up the lot online to re-confirm release data.'],
  ];
  ...
  return (
    <section className="section">
      <div className="container">
        ...
        <div className="v2-steps-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 0, borderTop: '1px solid var(--line)', borderBottom: '1px solid var(--line)' }}>
          {steps.map(([n, title, body], i) => (
            <Reveal key={n} delay={i * 80}>
              <div style={{ padding: '32px 20px', borderRight: i < steps.length - 1 ? '1px solid var(--line)' : 'none', minHeight: 220 }}>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--accent-hi)', marginBottom: 24 }}>{n}</div>
...
function Features() {
  const features = [
    ['In-House Synthesis', 'Solid-phase peptide sourcing and release documents reviewed before warehouse intake.', <Icon.beaker key="i" size={20} strokeWidth={1.5} />],
    ['HPLC-Verified Purity', 'Each batch is framed around analytical identity, purity, and batch release documentation.', <Icon.shield key="i" size={20} strokeWidth={1.5} />],
    ['COA With Every Vial', 'Each shipment includes the batch-specific certificate record. No exceptions.', <Icon.doc key="i" size={20} strokeWidth={1.5} />],
    ['Verified Buyers Only', 'A research-use attestation is required before restricted order access.', <Icon.link key="i" size={20} strokeWidth={1.5} />],
    ['Cold-Chain Shipping', 'Lyophilized materials packed cold and shipped to the registered lab address.', <Icon.download key="i" size={20} strokeWidth={1.5} />],
    ['Re-Verify Anytime', 'Enter the lot number to re-confirm release data online.', <Icon.qr key="i" size={20} strokeWidth={1.5} />],
  ];
  ...
  return (
    <section className="section" style={{ background: 'var(--bg-sunken)' }}>
      <div className="container">
        ...
        <div className="v2-features-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 0, border: '1px solid var(--line)', background: 'var(--bg-elevated)', borderRadius: 'var(--r-md)' }}>
  ...
  return (
    <section className="section">
      <div className="container">
        ...
        <div className="v2-preview-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
          {featured.map((item, i) => (
            <Reveal key={item.slug} delay={i * 40}>
              <Link href={`/products/${item.slug}`} className="card card-hover" style={{ padding: 16, display: 'block', cursor: 'pointer' }}>
                ...
                <div className="card-action">
                  <span style={{ color: 'var(--ok)' }}>· {item.stock} IN STOCK</span>
                  <span>VIEW LOT →</span>
                </div>
              </Link>
            </Reveal>
