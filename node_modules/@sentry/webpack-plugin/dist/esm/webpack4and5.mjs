...
import { CodeInjection, createComponentNameAnnotateHooks, createDebugIdUploadFunction, createSentryBuildPluginManager, generateModuleMetadataInjectorCode, generateReleaseInjectorCode, getDebugIdSnippet, stringToUUID } from "@sentry/bundler-plugin-core";
	...
	return function sentryWebpackPlugin(userOptions = {}) {
		...
		return { apply(compiler) {
			...
			compiler.hooks.afterEmit.tapAsync("sentry-webpack-plugin", (compilation, callback) => {
				...
				run().then(() => callback(), (err) => callback(err));
			});
...
//#endregion
export { sentryWebpackPluginFactory as t };
