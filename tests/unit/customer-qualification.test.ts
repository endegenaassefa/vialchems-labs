...
describe('customer qualification', () => {
  describe('ATTESTATIONS', () => {
    ...
    it('includes RUO research-only framing', () => {
      expect(ATTESTATIONS.some((a) => a.includes('in-vitro laboratory research'))).toBe(true);
    });
  ...
  describe('validateQualification', () => {
    const validInput = {
      ...
      researchPurpose: 'In-vitro studies on fibroblast migration in cell-culture wound-closure assays.',
    ...
    it('rejects invalid email', () => {
      const result = validateQualification({ ...validInput, email: 'not-an-email' });
      expect(result.ok).toBe(false);
    });

    it('rejects missing age acknowledgment', () => {
      const result = validateQualification({ ...validInput, ageAcknowledgment: false });
      expect(result.ok).toBe(false);
    });

    it('rejects missing RUO acknowledgment', () => {
      const result = validateQualification({ ...validInput, ruoAcknowledgment: false });
      expect(result.ok).toBe(false);
    });

    it('rejects missing jurisdiction acknowledgment', () => {
      const result = validateQualification({ ...validInput, jurisdictionAcknowledgment: false });
      expect(result.ok).toBe(false);
    });
    ...
    it('rejects research purpose with forbidden marketing pattern', () => {
      const result = validateQualification({
        ...validInput,
        researchPurpose: 'Studying weight loss applications for clients in our clinic.',
      });
      expect(result.ok).toBe(false);
    });

    it('rejects research purpose under 20 characters', () => {
      const result = validateQualification({ ...validInput, researchPurpose: 'too short' });
      expect(result.ok).toBe(false);
    });

    it('rejects unknown role', () => {
      const result = validateQualification({ ...validInput, role: 'rogue-role' });
      expect(result.ok).toBe(false);
    });
  });
});
