...
describe('assertMarketingCopySafe', () => {
  describe('forbidden patterns (must throw)', () => {
    const cases: Array<[string, string]> = [
      ['outcome claim — weight loss', 'BPC-157 supports weight loss in animals'],
      ['outcome claim — fat loss', 'helps with fat loss'],
      ['outcome claim — muscle growth', 'increases muscle growth'],
      ['outcome claim — appetite suppression', 'reduces appetite suppression effect'],
      ...
      ['catalog exclusion — semaglutide', 'studies on semaglutide pharmacokinetics'],
      ...
      ['quality claim — prescription strength', 'prescription strength formulation'],
      ...
      ['human-use intent — human consumption', 'not for human consumption is wrong context — this should still flag'],
      ['human-use intent — human dosing', 'human dosing protocols'],
      ['dosing protocol — dosing recommendation', 'follow our dosing recommendation'],
    ...
    it.each(cases)('throws on: %s', (_label, copy) => {
      expect(() => assertMarketingCopySafe(copy)).toThrow(/assertMarketingCopySafe violation/);
    });
  });
  ...
  describe('safe copy (must NOT throw)', () => {
    ...
    it.each(safeCases.map((s, i) => [`case ${i + 1}`, s]))('safe: %s', (_label, copy) => {
  ...
  describe('edge cases', () => {
    ...
    it('does not throw on non-string input', () => {
      expect(() => assertMarketingCopySafe(undefined as unknown as string)).not.toThrow();
      expect(() => assertMarketingCopySafe(null as unknown as string)).not.toThrow();
    });
  });
});
