...
(tooling ? describe : describe.skip)("Iron Law 2.45 — public/coa/ redaction gate", () => {
  it("OCRs every PDF in public/coa/ and confirms no forbidden lab branding survives", async () => {
    const files = (await fs.readdir(COA_DIR)).filter((f) => f.endsWith(".pdf"));
    if (files.length === 0) {
    ...
    for (const file of files) {
      ...
      try {
        execSync(`pdftoppm -png -r 200 -singlefile ${JSON.stringify(pdfPath)} ${JSON.stringify(tmpBase)}`, {
          stdio: "pipe",
        });
